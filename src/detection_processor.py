    ...
    def stop(self):
        ...
        if self.processor_thread:
            ...
            if self.processor_thread.is_alive():
                logger.error(
                    f"Detection processor thread did not stop after {THREAD_JOIN_TIMEOUT_SECONDS}s timeout - "
                    f"trying extended timeout. This may indicate a deadlock or blocking queue operation."
                )
                ...
                if self.processor_thread.is_alive():
                    ...
                    logger.warning(
                        "Detection processor thread stopped after extended timeout"
                    )
            else:
    ...
    def _processing_loop(self):
        ...
        while not self.stop_event.is_set():
            try:
                ...
                # Send to output queue with backpressure
                # Use blocking put with timeout instead of put_nowait to avoid silent data loss
                try:
                    self.output_queue.put(processed_result, timeout=QUEUE_PUT_TIMEOUT_SECONDS)
